...
def Wire.castAdd {inputCount gateCount : Nat} (extra : Nat) :
    Wire inputCount gateCount → Wire inputCount (gateCount + extra)
  | .input i => .input i
  | .gate j => .gate (Fin.castAdd extra j)

@[simp] theorem Wire.castAdd_input {inputCount gateCount : Nat} (extra : Nat)
    (i : Fin inputCount) : (Wire.input i : Wire inputCount gateCount).castAdd extra = .input i :=
  rfl

@[simp] theorem Wire.castAdd_gate {inputCount gateCount : Nat} (extra : Nat)
    (j : Fin gateCount) :
    (Wire.gate j : Wire inputCount gateCount).castAdd extra = .gate (Fin.castAdd extra j) :=
  rfl
