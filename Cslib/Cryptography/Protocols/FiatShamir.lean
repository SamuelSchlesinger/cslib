...
theorem FiatShamirSignature.correct
    ...
    (keyPair : ∀ n, R.Statement n × R.Witness n)
    (hkey : ∀ n, keyOf n (keyPair n).2 = (keyPair n).1) :
    (FiatShamirSignature P Message H keyOf).Correct keyPair := by
  intro n m r
  simp only [FiatShamirSignature]
  rw [← hkey]
  exact P.completeness n _ _ r _ (keyOf_valid n _)
