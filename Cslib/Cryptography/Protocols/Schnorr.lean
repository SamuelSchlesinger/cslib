...
theorem SchnorrSignature.correct (C : CyclicGroupFamily)
    ...
    (H : ∀ n, Message n → C.Group n → ZMod (C.order n))
    (keyPair : ∀ n, C.Group n × ZMod (C.order n))
    (hkey : ∀ n, C.gpow n (keyPair n).2 = (keyPair n).1) :
    (SchnorrSignature C Message H).Correct keyPair := by
  intro n m r
  simp only [SchnorrSignature, decide_eq_true_eq]
  rw [C.gpow_add, C.gpow_mul' n (keyPair n).2 _, hkey]
...
theorem SchnorrSignature_eq_FiatShamir (C : CyclicGroupFamily)
    ...
    SchnorrSignature C Message H =
      FiatShamirSignature (SchnorrProtocol C) Message H C.gpow := by
  rfl
