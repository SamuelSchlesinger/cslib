...
* `fiatShamir_ROM_secure` — asymptotic security: negligible advantage
  under negligible relation advantage, super-polynomial challenge
  space, negligible `δ`-unpredictable commitments, and polynomial
  query bound
...
private theorem uniformExpect_commit_collision_pair {R : EffectiveRelation}
    ...
    uniformExpect (Fin q → P.ProverRandomness n)
      (fun rs => if P.commit n w (keyOf n w) (rs i) =
        P.commit n w (keyOf n w) (rs j) then (1 : ℝ) else 0) ≤ δ n := by
  ...
  have h_comp : (fun rs : Fin q → P.ProverRandomness n =>
      if P.commit n w (keyOf n w) (rs i) =
        P.commit n w (keyOf n w) (rs j) then (1 : ℝ) else 0) =
    (fun p : P.ProverRandomness n × ({k : Fin q // k ≠ j} → P.ProverRandomness n) =>
      if P.commit n w (keyOf n w) p.1 =
        P.commit n w (keyOf n w) (p.2 ⟨i, hij⟩) then 1 else 0) ∘
  ...
  have h_bound : ∀ rest : {k : Fin q // k ≠ j} → P.ProverRandomness n,
      uniformExpect (P.ProverRandomness n) (fun rj =>
        if P.commit n w (keyOf n w) rj =
          P.commit n w (keyOf n w) (rest ⟨i, hij⟩) then 1 else 0) ≤ δ n :=
    fun rest => h_unpred n w (keyOf n w)
      (P.commit n w (keyOf n w) (rest ⟨i, hij⟩)) (keyOf_valid n w)
...
private theorem commitment_collision_bound {R : EffectiveRelation}
  ...
  have h_union : ∀ (w : R.Witness n) (rs : Fin q → P.ProverRandomness n),
      ...
      ∑ p : Fin q × Fin q,
        if p.1.val < p.2.val ∧
          P.commit n w (keyOf n w) (rs p.1) = P.commit n w (keyOf n w) (rs p.2)
          then 1 else 0 := by
    intro w rs
    split
    · next h =>
      ...
      let f : Fin q × Fin q → ℝ := fun p =>
        if p.1.val < p.2.val ∧
          P.commit n w (keyOf n w) (rs p.1) =
          P.commit n w (keyOf n w) (rs p.2) then 1 else 0
  ...
  have h_pair_bound : ∀ (w : R.Witness n) (p : Fin q × Fin q),
    ...
    · simp only [hij, true_and]
      exact uniformExpect_commit_collision_pair P keyOf keyOf_valid n q w δ
    ...
    · have : (fun rs : Fin q → P.ProverRandomness n =>
      ...
      exact le_trans (le_refl 0) (le_trans
        ...
        (h_unpred n w (keyOf n w)
          (P.commit n w (keyOf n w) (‹Nonempty (P.ProverRandomness n)›.some))
          (keyOf_valid n w)))
  -- Step 3: Assemble
...
private noncomputable def mapGame_real_run {R : EffectiveRelation}
    ...
    Option (Fin (A.numQueries n) × (Msg n × P.Commitment n × P.Response n)) :=
  fun ⟨w, rs⟩ ch =>
    mapGame_real_run_stmt P Msg A n w (keyOf n w) rs ch
...
private noncomputable def mapGame_real_advantage {R : EffectiveRelation}
  ...
  forkAcceptProb
    ...
    (P.Challenge n) q
    (mapGame_real_run P Msg keyOf A n)
...
private noncomputable def lazyRom_run {R : EffectiveRelation}
    ...
    Option (Fin (A.numQueries n) × (Msg n × P.Commitment n × P.Response n)) :=
  fun ⟨w, rs⟩ ch =>
    lazyRom_run_stmt P Msg A n w (keyOf n w) rs ch
...
private noncomputable def mapGame1_hvzk_run {R : EffectiveRelation}
    ...
    Option (Fin (A.numQueries n) × (Msg n × P.Commitment n × P.Response n)) :=
  fun ⟨w, sr⟩ ch =>
    mapGame1_hvzk_run_stmt P Msg hvzk A n (keyOf n w) sr ch
...
private noncomputable def mapGame1_hvzk_advantage {R : EffectiveRelation}
  ...
  forkAcceptProb
    ...
    (P.Challenge n) q
    (mapGame1_hvzk_run P Msg keyOf hvzk A n)
...
private theorem mapGame_real_eq_mapGame1_hvzk {R : EffectiveRelation}
    ...
    mapGame1_hvzk_advantage P Msg keyOf hvzk A n := by
  ...
  exact runWithState_uniformExpect_oracle_eq q (A.interact n y)
    ...
    (by
      ...
      | inl m =>
        ...
        simp only [mapGameRealOracle, mapGame1HvzkOracle]
        exact hvzk.sim_distribution n w y (ch i) (keyOf_valid n w)
...
private theorem lazyPairCommitReuse_pair_bound {R : EffectiveRelation}
    ...
    uniformExpect
      ((R.Witness n × (Fin (A.numQueries n) → P.ProverRandomness n)) ×
        (Fin (A.numQueries n) → P.Challenge n))
  (fun ⟨⟨w, rs⟩, ch⟩ =>
    if lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch i j then (1 : ℝ) else 0)
  ≤ δ n := by
  classical
  let q := A.numQueries n
  ...
  have h_reorder :
      ...
      uniformExpect (R.Witness n) (fun w =>
        uniformExpect (Fin q → P.Challenge n) (fun ch =>
          uniformExpect (Fin q → P.ProverRandomness n) (fun rs =>
            if lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch i j then (1 : ℝ) else 0))) := by
    rw [uniformExpect_prod, uniformExpect_prod]
    congr 1
    ext w
  ...
  have h_inner : ∀ (w : R.Witness n) (ch : Fin q → P.Challenge n),
    ...
    have h_rest :
      ...
      have hδ_nonneg : 0 ≤ δ n := by
        have htmp := h_unpred n w y (P.commit n w y r0) (keyOf_valid n w)
          ...
          | inl mj =>
            ...
            rw [show (fun rj => g (e.symm (rj, rest))) =
                (fun rj => if ti = P.commit n w y rj then (1 : ℝ) else 0) from by
                  funext rj; exact h_point rj]
            simpa [eq_comm] using h_unpred n w y ti (keyOf_valid n w)
...
private theorem lazyCommitReuse_bound {R : EffectiveRelation}
    ...
    uniformExpect
      ...
      (fun ⟨⟨w, rs⟩, ch⟩ =>
        if ∃ (i j : Fin (A.numQueries n)), i.val < j.val ∧
            lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch i j
        then (1 : ℝ) else 0)
  ...
  have h_union : ∀ (w : R.Witness n) (rs : Fin q → P.ProverRandomness n)
      ...
      ∑ p : Fin q × Fin q,
        if p.1.val < p.2.val ∧
            lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch p.1 p.2
        then 1 else 0 := by
    intro w rs ch
    split
    · rename_i h
  ...
  have h_pair : ∀ (p : Fin q × Fin q),
    ...
    rcases p with ⟨i, j⟩
    by_cases hij : i.val < j.val
    · have := lazyPairCommitReuse_pair_bound P Msg keyOf keyOf_valid A n δ h_unpred i j hij
      exact le_trans
        (uniformExpect_mono _ (fun ⟨⟨w, rs⟩, ch⟩ => by
          by_cases hpair : lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch i j
          · simp [hij, hpair]
          · simp [hij, hpair]))
        this
    · have h_zero :
          (fun x :
            (R.Witness n × (Fin q → P.ProverRandomness n)) × (Fin q → P.Challenge n) =>
              if i.val < j.val ∧
                  lazyPairCommitReuse P Msg A n x.1.1 (keyOf n x.1.1) x.1.2 x.2 i j
              then (1 : ℝ) else 0) = fun _ => 0 := by
        funext x
        simp [hij]
      ...
      have hδ_nonneg : 0 ≤ δ n := by
        ...
        have htmp := h_unpred n (Classical.arbitrary (R.Witness n))
          (keyOf n (Classical.arbitrary (R.Witness n)))
          (P.commit n (Classical.arbitrary (R.Witness n))
            (keyOf n (Classical.arbitrary (R.Witness n))) r0)
          (keyOf_valid n (Classical.arbitrary (R.Witness n)))
        exact le_trans (uniformExpect_nonneg _ (fun _ => by split <;> norm_num)) htmp
      exact hδ_nonneg
  calc uniformExpect
        ((R.Witness n × (Fin q → P.ProverRandomness n)) × (Fin q → P.Challenge n))
...
private theorem rom_eq_lazy_rom {R : EffectiveRelation}
      ...
      | some (queries, (mf, tf, zf), finalMap) =>
        ...
        if hj : j < A.numQueries n then
          let signMsgs := queries.filterMap (fun q => match q with
            | .inl m => some m | .inr _ => none)
          match assocLookup (mf, tf) finalMap with
          | some c => boolToReal (P.verify n (keyOf n w) tf c zf && !decide (mf ∈ signMsgs))
          | none => 0
        else 0
  have h_swap :=
    lazyRomH_runWithState_uniform_eq (P := P) (Msg := Msg)
      n (A.numQueries n) (A.interact n (keyOf n w)) w (keyOf n w) rs
      runPayoff
  let rhsNested : (Fin (A.numQueries n) → P.Challenge n) → ℝ := fun ch =>
    match
      (match
        (A.interact n (keyOf n w)).runWithState (A.numQueries n)
          (lazyRomOracle P Msg n (A.numQueries n) w (keyOf n w) rs ch) [] with
      ...
      | some (queries, (mf, tf, zf), finalMap) =>
        if hj : List.findIdx (fun x => decide (x = Sum.inr (mf, tf))) queries < A.numQueries n then
          match assocLookup (mf, tf) finalMap with
          | some c =>
            if P.verify n (keyOf n w) tf c zf &&
  ...
  have h_rhs_fun :
      (fun ch =>
        runPayoff ((A.interact n (keyOf n w)).runWithState (A.numQueries n)
          (lazyRomOracle P Msg n (A.numQueries n) w (keyOf n w) rs ch) [])) =
      rhsNested := by
    funext ch
    let result :=
      (A.interact n (keyOf n w)).runWithState (A.numQueries n)
      ...
      uniformExpect (Fin (A.numQueries n) → (Msg n × P.Commitment n → P.Challenge n))
        (fun Hs =>
          runPayoff
            ((A.interact n (keyOf n w)).runWithState (A.numQueries n)
              (fun i st qry =>
                lazyRomHOracle P Msg n (A.numQueries n) w (keyOf n w) rs i (Hs i) st qry) [])) =
    ...
    calc
      uniformExpect (Fin (A.numQueries n) → (Msg n × P.Commitment n → P.Challenge n))
        ...
        uniformExpect (Fin (A.numQueries n) → P.Challenge n)
          (fun ch =>
            runPayoff ((A.interact n (keyOf n w)).runWithState (A.numQueries n)
              (lazyRomOracle P Msg n (A.numQueries n) w (keyOf n w) rs ch) [])) := by
  ...
  let goalRhs : (Fin (A.numQueries n) → P.Challenge n) → ℝ := fun b =>
    match
      match
        (A.interact n (keyOf n w)).runWithState (A.numQueries n)
          (lazyRomOracle P Msg n (A.numQueries n) w (keyOf n w) rs b) [] with
      | none => (none : Option (Fin (A.numQueries n) ×
          (Msg n × P.Commitment n × P.Response n)))
      | some (queries, (mf, tf, zf), finalMap) =>
  ...
  have h_rhs_nested_eq_goal : rhsNested = goalRhs := by
    ...
    let result :=
      (A.interact n (keyOf n w)).runWithState (A.numQueries n)
        (lazyRomOracle P Msg n (A.numQueries n) w (keyOf n w) rs b) []
    cases h_result : result with
    | none =>
...
private theorem lazy_le_mapGame_real {R : EffectiveRelation}
  ...
  let fL : Ω → ℝ := fun ⟨⟨w, rs⟩, ch⟩ =>
    match lazyRom_run_stmt P Msg A n w (keyOf n w) rs ch with
    | none => 0
    | some _ => 1
  let fM : Ω → ℝ := fun ⟨⟨w, rs⟩, ch⟩ =>
    match mapGame_real_run_stmt P Msg A n w (keyOf n w) rs ch with
    | none => 0
    | some _ => 1
  let bad : Ω → Prop := fun ⟨⟨w, rs⟩, ch⟩ =>
    ∃ (i j : Fin q), i.val < j.val ∧
      lazyPairCommitReuse P Msg A n w (keyOf n w) rs ch i j = true
  have h_agree : ∀ ω : Ω, ¬ bad ω → fL ω = fM ω := by
    intro ⟨⟨w, rs⟩, ch⟩ hnb
    dsimp [fL, fM, bad] at hnb ⊢
    rw [lazy_run_stmt_eq_mapGame_real_run_stmt_of_no_reuse
      P Msg A n w (keyOf n w) rs ch hnb]
  ...
  have h_bad_bound :
      ...
      (q : ℝ) ^ 2 * δ n := by
    simpa [Ω, bad] using
      lazyCommitReuse_bound P Msg keyOf keyOf_valid A n δ h_unpred
  have h_sub :
...
private theorem rom_le_mapGame_real {R : EffectiveRelation}
    ...
    mapGame_real_advantage P Msg keyOf A n +
    (A.numQueries n : ℝ) ^ 2 * δ n := by
  have h1 := rom_eq_lazy_rom P Msg keyOf A n
  have h2 := lazy_le_mapGame_real P Msg keyOf keyOf_valid A n δ h_unpred
  linarith
...
private theorem rom_eq_mapGame1_hvzk_bound {R : EffectiveRelation}
    ...
    (h_unpred : P.UnpredictableCommitments δ) :
    (ROM_EUF_CMA_Game P Msg keyOf).advantage A n ≤
      mapGame1_hvzk_advantage P Msg keyOf hvzk A n +
      (A.numQueries n : ℝ) ^ 2 * δ n := by
  have h1 := rom_le_mapGame_real P Msg keyOf keyOf_valid A n δ h_unpred
  have h2 := mapGame_real_eq_mapGame1_hvzk P Msg keyOf hvzk keyOf_valid A n
  linarith
...
private theorem forkExtraction_le_advR_map {R : EffectiveRelation}
  ...
  have h_mono : forkProb
    ...
    rcases h_run₁ : mapGame1_hvzk_run_stmt P Msg hvzk A n (keyOf n w) sr ch₁
      with _ | ⟨j, mf₁, tf₁, zf₁⟩
    · exact boolToReal_nonneg _
    · dsimp only []
      rcases h_run₂ : mapGame1_hvzk_run_stmt P Msg hvzk A n (keyOf n w) sr
          (fun i => if i.val < j.val then ch₁ i else ch₂ i)
        with _ | ⟨j', mf₂, tf₂, zf₂⟩
      · exact boolToReal_nonneg _
      · dsimp only []
...
private theorem mapGame1_hvzk_forking_bound {R : EffectiveRelation}
  ...
  · refine ⟨fun _ => Classical.arbitrary _, ?_⟩
    have h_adv_le : mapGame1_hvzk_advantage P Msg keyOf hvzk A n ≤ 0 := by
      ...
      suffices h : forkAcceptProb
          (R.Witness n × (Fin (A.numQueries n) → hvzk.SimRandomness n))
          (P.Challenge n) (A.numQueries n)
          (mapGame1_hvzk_run P Msg keyOf hvzk A n) ≤ 0 from h
      ...
      · apply uniformExpect_mono
        ...
        cases h_run : mapGame1_hvzk_run P Msg keyOf hvzk A n ⟨w, sr⟩ ch with
        | none => norm_num
  ...
  · have hq_pos : 0 < A.numQueries n := by omega
    ...
    let run := mapGame1_hvzk_run P Msg keyOf hvzk A n
    have h_fork := forking_lemma Coins (P.Challenge n) (A.numQueries n) run hq_pos
    obtain ⟨find_n, h_extract⟩ := forkExtraction_le_advR_map P Msg keyOf ss hvzk A n
    have h_rearrange :
...
theorem fiatShamir_ROM_bound {R : EffectiveRelation}
    ...
    ∃ B : RelationSolver R, ∀ n,
      (ROM_EUF_CMA_Game P Msg keyOf).advantage A n ≤
        ...
        (A.numQueries n : ℝ) ^ 2 * δ n := by
  obtain ⟨B, hB⟩ := mapGame1_hvzk_forking_bound P Msg keyOf ss hvzk keyOf_valid A
  exact ⟨B, fun n => by
    have h_rom_le := rom_eq_mapGame1_hvzk_bound P Msg keyOf hvzk keyOf_valid A n δ h_unpred
    have h_fork := hB n
    linarith⟩
...
theorem fiatShamir_ROM_secure {R : EffectiveRelation}
  ...
  -- Use Map-based proof chain (independent of SimGame)
  obtain ⟨B, hB⟩ := mapGame1_hvzk_forking_bound P Msg keyOf ss hvzk keyOf_valid A
