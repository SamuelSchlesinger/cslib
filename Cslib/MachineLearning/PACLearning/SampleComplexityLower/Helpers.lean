...
The lower-bound argument works on the **marginal sample space** `Fin m → α`
...
under a realizable distribution `D = P.map (x ↦ (x, c x))` is handled at the
top-level theorem (`SampleComplexityLower.lean`) via
`pi_map_sampleOf` below and the bridge lemma
`error_map_eq_hypothesisError` from `PACLearning.Defs`.

## Main definitions

- `sampleOf c xs`: build a labeled sample from points and a set-valued concept
  (the realizable labeling `(xs i, decide (xs i ∈ c))`).
...
- `hypothesisError_eq_of_inter_eq`: error invariance under same intersection
  with the support `W`.
- `pi_measure_compl_zero`: the product measure vanishes off `W^m`.
- `nullMeasurableSet_pi_of_finite_support`: null-measurability from finite
  support.
- `measurableSet_setOf_exists_pi_eq`: the set of samples containing a given
  point is measurable.
...
The pi-product↔pushforward bridge `pi_map_sampleOf`, which translates
joint-sample-space failure events to the marginal sample space at the top of
the EHKV proof, lives in `SampleComplexityLower.lean` itself, near where it is
used.
-/
...
open Classical in
/-- Construct a labeled sample from a sequence of points `xs` and a set-valued
concept `c`. Each point is labeled `true` if it belongs to `c`, else `false`. -/
def sampleOf {α : Type*} {m : ℕ} (c : Set α) (xs : Fin m → α) :
    LabeledSample α Bool m :=
  fun i => (xs i, decide (xs i ∈ c))

open Classical in
/-- The set of elements of a `Finset` that appear in a sample. -/
def seenElements {α : Type*} (W' : Finset α) {m : ℕ}
    (xs : Fin m → α) : Finset α :=
...
/-- If a measure `P` on `α` gives zero mass to the complement of a finite set
`W`, then the product measure `P^m` gives zero mass to the complement of
`W^m`. -/
...
/-- If a measure `P` on `α` gives zero mass to the complement of a finite set
`W`, then every set in the product space is `NullMeasurableSet`. -/
...
open Classical in
/-- The cardinality of `seenElements W' xs` as an extended non-negative real equals
a finset sum of indicator functions over `W'`. -/
...
theorem measurable_seenElements_card
    {α : Type*} [MeasurableSpace α] [MeasurableSingletonClass α] {m : ℕ}
    (W' : Finset α) :
...
theorem expected_seenElements_le
  ...
  calc ∑ w ∈ W', μ {xs | ∃ i, xs i = w}
      ≤ ∑ _w ∈ W', ENNReal.ofReal (↑m * p) := by
        ...
        linarith [one_add_mul_sub_le_pow (show (-1 : ℝ) ≤ 1 - p by linarith) m]
