...
- `concepts_agree_of_seen_inter`: two concepts agreeing on seen elements yield the
  same labeled sample.
...
theorem concepts_agree_of_seen_inter
    {W W' : Finset α} {w₀ : α}
    {m : ℕ} {xs : Fin m → α}
...
`8ε'/d` and `|U| ≥ d/2`. -/
theorem unseen_measure_ge [MeasurableSingletonClass α]
    {U : Finset α} {d : ℕ} {ε' : ℝ} {P : Measure α}
