...
Slices of languages built from slices, constant-size families for the empty language under two
readings of the output, unary languages in P/poly, and compatibility of the Shannon and Lupanov
bounds for families.
-/
...
example {L : Language Bool} (hL : L ∈ SIZE fun n => 3 * n ^ 2 + 5) : L ∈ PPoly :=
...
example (ε : ℝ) (hε : 0 < ε) :
    ∃ L : Language Bool, ∃ N : ℕ,
      (∀ F : CircuitFamily signature, F.Decides interpretation id L →
